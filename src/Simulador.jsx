...
function calcSim(g) {
  ...
  // Lógica: a redução (reducaoPct%) incide SOMENTE sobre o crédito líquido (sem taxas).
  // As taxas (adm + fundo de reserva) permanecem integrais sobre o crédito original.
  //
  // Fórmula correta:
  //   parcelaAmortização  = credito / prazo
  //   parcelaTaxas        = credito * (taxa + fundo) / 100 / prazo
  //   parcelaAmortizaçãoReduzida = parcelaAmortização * (1 − reducaoPct / 100)
  //   plReduzida = parcelaAmortizaçãoReduzida + parcelaTaxas
  //
  // Equivalente a: (credito * (1 − reducaoPct/100) + taxasAbsolutas) / prazo
  // A parcela reduzida é sempre calculada sobre ps (pré-lance); o lance é evento pós-contemplação.
  const taxasAbsolutas  = credito * ((taxa || 0) / 100 + (fundoReserva || 0) / 100);
  const creditoReduzido = credito * (1 - (reducaoPct || 0) / 100);
  const plReduzida      = (reducao && reducaoPct > 0)
    ? (creditoReduzido + taxasAbsolutas) / prazo
    : ps; // fallback correto: parcela cheia pré-lance (não plCetReal pós-lance)

  // pl = parcela pós-lance (usada em custo total e exibição do pipeline)
  const pl = plCetReal;
            ...
            {g.credito > 0 && g.prazo > 0 && (
              <div style={{ marginTop: 10, padding: '8px 10px', background: 'rgba(255,255,255,.03)', borderRadius: 7, fontSize: 11 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 4, textAlign: 'center' }}>
                  <div>
                    <div style={{ color: 'var(--muted)', marginBottom: 2 }}>Parcela</div>
                    <div style={{ color: 'var(--gold)', fontWeight: 600 }}>{R(sim.ps)}</div>
                  </div>
                  {g.reducao && g.reducaoPct > 0 && (
                    <div>
                      <div style={{ color: 'var(--muted)', marginBottom: 2 }}>Reduzida</div>
                      <div style={{ color: 'var(--green)', fontWeight: 600 }}>{R(sim.plReduzida)}</div>
                    </div>
                  )}
                  ...
                  {!sim.lancePct && !g.reducao && <div><div style={{ color: 'var(--muted)', marginBottom: 2 }}>Custo Total</div><div style={{ fontWeight: 600 }}>{R(sim.tl)}</div></div>}
