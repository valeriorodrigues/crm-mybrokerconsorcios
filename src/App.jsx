...
import {
  ...
  ChevronDown, MessageCircle, BarChart3, Calculator, Upload, Download, LogOut
} from "lucide-react";

import { PRODUCTS, STAGES, PIPELINE_STAGES, STAGE_COLORS, PRODUCT_COLORS, DEFAULT_TEMPLATES } from "./utils/constants";
import {
  uid, today, calcScore, formatCurrency, formatDate,
...
import { pixelLead, pixelContact, pixelPurchase, pixelLost, pixelStageChange, pixelInitiateCheckout } from "./utils/pixel";

import { SearchBox, ConfirmModal, PromptModal } from "./components/UI";
...
export default function App() {
  ...
  // ─── Dialog state (substitui confirm/prompt nativos) ───
  const [dialog, setDialog] = useState(null);
  // dialog = { type: "confirm"|"prompt", message, confirmLabel, danger, onConfirm, placeholder }

  const showConfirm = (message, onConfirm, { confirmLabel = "Confirmar", danger = true } = {}) =>
    setDialog({ type: "confirm", message, confirmLabel, danger, onConfirm });

  const showPrompt = (message, onConfirm, { placeholder = "" } = {}) =>
    setDialog({ type: "prompt", message, onConfirm, placeholder });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Load settings
        try {
          const sv = await db.getSettings();
          if (sv) setSettings(sv);
          else {
            const local = localStorage.getItem("mybroker_settings");
  ...
  }, [user]); // ← depende de user: dispara quando user muda de null para objeto
  ...
  const saveContact = (c) => {
    ...
    db.saveContact(c);
    if (isNew) pixelLead(c); // novo contato = Lead para a Meta
  };
  const deleteContact = (id) => {
    showConfirm("Excluir este contato? Esta ação não pode ser desfeita.", () => {
  ...
  const deleteCompany = (id) => {
    showConfirm("Excluir esta empresa?", () => {
  ...
  const saveNeg = (n) => {
    const isNew = !negotiations.find(x => x.id === n.id);
    ...
    if (isNew) pixelInitiateCheckout(n); // nova negociação = início do checkout
  };
  const deleteNeg = (id) => {
    showConfirm("Excluir esta negociação? Atividades e tarefas vinculadas também serão removidas.", () => {
  ...
  const saveActivity = (a) => {
    ...
    setModal(null);
    db.saveActivity(a);
    // Dispara evento de contato para canais reais (não Sistema)
    if (a.channel && a.channel !== "Sistema") pixelContact(a.channel);
  };
  const saveTask = (t) => {
  ...
  const changeStage = (id, stage) => {
    ...
    db.saveActivity(act);
    if (neg) pixelStageChange(neg, stage); // avançou no funil
  };
  ...
  const markWon = (id) => {
    ...
    db.saveActivity(act);
    if (neg) pixelPurchase(neg); // 🏆 evento mais importante — venda fechada
  };

  const markLost = (id) => {
    showPrompt("Motivo da perda:", (reason) => {
      const neg = negotiations.find(n => n.id === id);
      ...
      db.saveActivity(act);
      if (neg) pixelLost(neg, reason); // perda registrada
    }, { placeholder: "Ex: Preço, prazo, sem interesse..." });
  };

  // Handle kanban drop — agora via @hello-pangea/dnd diretamente no NegotiationPipeline

  // ─── Navigation ───
  const navigate = (pg) => { setPage(pg); setSelected(null); };

  // ─── Auth gate ───
  if (!user) return <LoginScreen onLogin={(u) => { init.current = false; setReady(false); setUser(u); }} />;
    ...
    <>
        ...
        <aside className="sidebar">
          ...
          <div className="sidebar-footer">
            ...
            <div style={{ borderTop: "1px solid var(--bd)", marginTop: 8, paddingTop: 8, display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <label className="btn btn-sm" style={{ width: "100%", justifyContent: "center", fontSize: 10, cursor: "pointer" }}>
                ...
                <input type="file" accept=".json" style={{ display: "none" }} onChange={async (e) => {
                  ...
                  showConfirm(
                    "Restaurar backup? Isso substituirá TODOS os dados atuais.",
                    async () => {
                      try {
                      ...
                      } catch (err) { console.error("Erro ao restaurar:", err.message); }
                    },
            ...
            <div style={{ borderTop: "1px solid var(--bd)", marginTop: 8, paddingTop: 8 }}>
              <div style={{ fontSize: 10, color: "var(--tx3)", marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {user?.email}
              </div>
              <button className="btn btn-sm" style={{ width: "100%", justifyContent: "center", fontSize: 10, color: "var(--red)", borderColor: "rgba(239,68,68,.3)" }}
                onClick={() => showConfirm("Sair da sua conta?", async () => {
                await auth.signOut();
                init.current = false;  // permite recarregar ao logar novamente
                setReady(false);
                setContacts([]); setCompanies([]); setNegotiations([]);
                setActivities([]); setTasks([]); setTemplates([]);
                setUser(null);
          ...
          <div className="content" onClick={() => createOpen && setCreateOpen(false)}>
            ...
            {page === "contacts" && selected && (
              <ContactDetail
                ...
                onDelete={deleteContact}
                onCreateNeg={() => {
                  setEditTarget({ ...emptyNegotiation(), contactId: selected.id, companyId: selected.companyId || "" });
                  setModal("newNeg");
                }}
      ...
      {modal === "settings" && (
        <SettingsModal
          ...
          onSave={(s) => {
            setSettings(s);
            localStorage.setItem("mybroker_settings", JSON.stringify(s));
            db.saveSettings(s).catch(() => {}); // persist to Supabase
            setModal(null);
          }}
      ...
      {/* ═══ DIALOGS (substitui confirm/prompt nativos) ═══ */}
      {dialog?.type === "confirm" && (
        <ConfirmModal
