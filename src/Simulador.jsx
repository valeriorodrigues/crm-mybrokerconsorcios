...
// ─── CALCULATIONS ────────────────────────────────────────────────────────────────
function calcSim(g) {
  const { credito = 200000, taxa = 9.2, prazo = 84, lanceProprio = 0, lanceEmbutido = 0, fundoReserva = 0, reducao = false, reducaoPct = 0 } = g;
  const lancePct = (lanceProprio || 0) + (lanceEmbutido || 0);
  const ps = PMT(credito, taxa, prazo); // parcela sem lance
  const lv = credito * lancePct / 100; // lance total
  ...
  // Parcela pós-lance (sobre crédito menos lance)
  const creditoLiquido = credito - lv; // PV = o que o cliente "leva" de fato
  const pl = creditoLiquido > 0 ? PMT(credito, taxa, prazo) : 0; // parcela sobre crédito total (taxas incidem sobre total)
  // Nota: a parcela no consórcio incide sobre o crédito total, não sobre o líquido.
  // O lance reduz o número de parcelas restantes ou o saldo devedor, mas a taxa incide sobre o bem.
  // Para o CET correto, usamos: PV = crédito - lance, PMT = parcela efetiva, n = prazo restante.

  const plReduzida = reducao && reducaoPct > 0 ? pl * (1 - reducaoPct / 100) : pl;
  const tl = lv + pl * prazo; // custo total nominal
  const anos = prazo / 12;

  // ═══ CET CORRETO via TIR (Newton-Raphson) ═══
  // O CET mede o custo sobre o capital de terceiros (crédito líquido)
  // PV = Crédito - Lance (o que o cliente recebe da administradora)
  // PMT = parcela mensal (o que o cliente paga)
  // n = prazo em meses
  let cetMensal = 0;
  let cetAnual = 0;
  if (creditoLiquido > 0 && pl > 0 && prazo > 0) {
    cetMensal = calcTIR(creditoLiquido, pl, prazo);
    cetAnual = (Math.pow(1 + cetMensal, 12) - 1); // composição anual
  }
  ...
  return {
    ps, lv, lvProprio, lvEmbutido, fr, pl, plReduzida, tl,
    cetMensal: cetMensal * 100, // em percentual
    cetAnual: cetAnual * 100,   // em percentual
    creditoLiquido,
...
export default function Simulador() {
  ...
  function newProp() {
    setCurrent({
      ...
      grupo: {
        nome: '', prazo: 200, taxa: 18, credito: 200000,
      ...
      {proposals.length === 0 ? (
        ...
        <div key={p.id} className="li" onClick={() => onOpen(p)}>
          ...
          <div className="li-info">
            ...
            <div className="li-meta">{p.administradora || '—'} · {PRODUTOS[p.produto.tipo]?.label || '—'} · {p.grupo.prazo}m · {p.id} · {fmt(p.createdAt)}</div>
          </div>
...
function CreateView({ data, step, contacts, onChange, onStep, onSave, onCancel, showToast }) {
  ...
  return (
    <div>
      ...
      {step === 4 && <StepFormato data={data} upd={upd} />}
...
// ─── STEP 3: SIMULAÇÃO (grupo, valores, lances, redução) ────────────────────────
function StepSimulacao({ data, upd, onChange }) {
  ...
  return (
    <div>
      ...
      <div className="sec-sub">Dados do grupo e parâmetros do consórcio</div>

      <div className="g2">
        <div className="field"><label className="lbl">Grupo</label><input className="inp" value={g.nome} onChange={e => upd('grupo.nome', e.target.value)} placeholder="Ex: 1234" /></div>
        <div className="field"><label className="lbl">Valor do Bem (R$)</label><input className="inp" type="number" value={g.credito} onChange={e => upd('grupo.credito', Number(e.target.value))} /></div>
      </div>
      <div className="g2">
        <div className="field"><label className="lbl">Prazo (meses)</label><input className="inp" type="number" value={g.prazo} onChange={e => upd('grupo.prazo', Number(e.target.value))} /></div>
        <div className="field"><label className="lbl">Taxa de Administração (%)</label><input className="inp" type="number" step="0.01" value={g.taxa} onChange={e => upd('grupo.taxa', Number(e.target.value))} /></div>
      </div>
      <div className="g2">
        <div className="field"><label className="lbl">Fundo de Reserva (%)</label><input className="inp" type="number" step="0.01" value={g.fundoReserva || 0} onChange={e => upd('grupo.fundoReserva', Number(e.target.value))} /></div>
        <div className="field"><label className="lbl">Lance Recursos Próprios (%)</label><input className="inp" type="number" step="0.1" value={g.lanceProprio || 0} onChange={e => upd('grupo.lanceProprio', Number(e.target.value))} /></div>
      </div>
      <div className="g2">
        <div className="field"><label className="lbl">Lance Embutido (%)</label><input className="inp" type="number" step="0.1" value={g.lanceEmbutido || 0} onChange={e => upd('grupo.lanceEmbutido', Number(e.target.value))} /></div>
        <div className="field" />
      </div>
      ...
      <div className="tog-row">
        <div>
          <div className="tog-label">Redução de parcela até contemplação</div>
          <div className="tog-desc">Reduz o valor da parcela mensal até ser contemplado</div>
        </div>
        <button className={`tog ${g.reducao ? 'on' : ''}`} onClick={() => upd('grupo.reducao', !g.reducao)} />
      </div>
      {g.reducao && (
        <div className="field" style={{ marginTop: 8 }}>
          <label className="lbl">Percentual de Redução (%)</label>
          <input className="inp" type="number" step="0.1" value={g.reducaoPct || 0} onChange={e => upd('grupo.reducaoPct', Number(e.target.value))} />
        </div>
      )}

      {/* Resumo da simulação */}
      {g.credito > 0 && g.prazo > 0 && (
        <div className="sec-block" style={{ marginTop: 14 }}>
          <div className="sec-block-title">Resumo da Simulação</div>
          <div className="kv"><span className="kv-l">Valor do Bem</span><span className="kv-r">{R(g.credito)}</span></div>
          <div className="kv"><span className="kv-l">Prazo</span><span className="kv-r">{g.prazo} meses ({(g.prazo / 12).toFixed(1)} anos)</span></div>
          <div className="kv"><span className="kv-l">Taxa de Administração</span><span className="kv-r">{Pt(g.taxa)}</span></div>
          <div className="kv"><span className="kv-l">Fundo de Reserva</span><span className="kv-r">{R(sim.fr)}</span></div>
          <div className="kv"><span className="kv-l">Parcela Mensal</span><span className="kv-r" style={{ color: 'var(--gold)' }}>{R(sim.ps)}</span></div>
          {g.reducao && <div className="kv"><span className="kv-l">Parcela Reduzida</span><span className="kv-r" style={{ color: 'var(--green)' }}>{R(sim.plReduzida)}</span></div>}
          {sim.lancePct > 0 && <>
            <div className="kv"><span className="kv-l">Lance Total ({Pt(sim.lancePct)})</span><span className="kv-r">{R(sim.lv)}</span></div>
            {sim.lvProprio > 0 && <div className="kv"><span className="kv-l">  └ Recursos Próprios</span><span className="kv-r">{R(sim.lvProprio)}</span></div>}
            {sim.lvEmbutido > 0 && <div className="kv"><span className="kv-l">  └ Embutido</span><span className="kv-r">{R(sim.lvEmbutido)}</span></div>}
            <div className="kv"><span className="kv-l">Parcela Após Lance</span><span className="kv-r" style={{ color: 'var(--green)' }}>{R(sim.pl)}</span></div>
          </>}
          <div className="kv"><span className="kv-l">Custo Total</span><span className="kv-r">{R(sim.tl)}</span></div>
          {sim.cetMensal > 0 && <>
            <div className="kv" style={{ marginTop: 6, borderTop: '1px solid rgba(255,255,255,.06)', paddingTop: 6 }}>
              <span className="kv-l">Crédito Líquido (Crédito − Lance)</span><span className="kv-r">{R(sim.creditoLiquido)}</span>
            </div>
            <div className="kv"><span className="kv-l">CET Mensal (TIR)</span><span className="kv-r" style={{ color: 'var(--gold)' }}>{Pt(sim.cetMensal, 3)} a.m.</span></div>
            <div className="kv"><span className="kv-l">CET Anual (TIR)</span><span className="kv-r" style={{ color: 'var(--gold)', fontSize: 15, fontWeight: 700 }}>{Pt(sim.cetAnual, 2)} a.a.</span></div>
          </>}
        </div>
      )}
    </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 14 }}>
        {TIPOS_PROP.map(t => (
          <div key={t.id} className={`card card-click${data.tipoProposta === t.id ? ' card-sel' : ''}`}
            ...
            <div><div style={{ fontSize: 13, fontWeight: 600 }}>{t.label}</div><div style={{ fontSize: 10, color: 'var(--muted)' }}>{t.desc}</div></div>
          </div>
        ))}
      </div>
      <div className="sec-title" style={{ fontSize: 13, marginTop: 14 }}>Opções</div>
      <div className="tog-row"><div><div className="tog-label">Comparativo Financiamento</div><div className="tog-desc">CEF e Banco vs Consórcio</div></div><button className={`tog ${data.opcoes.comparativo?'on':''}`} onClick={()=>upd('opcoes.comparativo',!data.opcoes.comparativo)}/></div>
      <div className="tog-row"><div><div className="tog-label">CET Estimado</div></div><button className={`tog ${data.opcoes.cet?'on':''}`} onClick={()=>upd('opcoes.cet',!data.opcoes.cet)}/></div>
      <div className="tog-row"><div><div className="tog-label">Valorização INCC</div><div className="tog-desc">Projeção de valorização do bem</div></div><button className={`tog ${data.opcoes.incc?'on':''}`} onClick={()=>upd('opcoes.incc',!data.opcoes.incc)}/></div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="sec-block">
        ...
        {p.grupo.nome && <div className="kv"><span className="kv-l">Grupo</span><span className="kv-r">{p.grupo.nome}</span></div>}
        <div className="kv"><span className="kv-l">Valor do Bem</span><span className="kv-r" style={{ color: 'var(--gold)', fontSize: 15 }}>{R(p.grupo.credito)}</span></div>
        <div className="kv"><span className="kv-l">Prazo</span><span className="kv-r">{p.grupo.prazo} meses</span></div>
        <div className="kv"><span className="kv-l">Taxa de Administração</span><span className="kv-r">{Pt(p.grupo.taxa)}</span></div>
        {p.grupo.fundoReserva > 0 && <div className="kv"><span className="kv-l">Fundo de Reserva</span><span className="kv-r">{Pt(p.grupo.fundoReserva)} ({R(sim.fr)})</span></div>}
      </div>

      <div className="sec-block">
        <div className="sec-block-title">Simulação</div>
        ...
        {p.grupo.reducao && <div className="kv"><span className="kv-l">Parcela Reduzida (até contemplação)</span><span className="kv-r" style={{ color: 'var(--green)' }}>{R(sim.plReduzida)} (-{Pt(p.grupo.reducaoPct)})</span></div>}
        {sim.lancePct > 0 && <>
          <div className="kv"><span className="kv-l">Lance Total</span><span className="kv-r">{R(sim.lv)} ({Pt(sim.lancePct)})</span></div>
          {sim.lvProprio > 0 && <div className="kv"><span className="kv-l">  Recursos Próprios</span><span className="kv-r">{R(sim.lvProprio)} ({Pt(p.grupo.lanceProprio)})</span></div>}
          {sim.lvEmbutido > 0 && <div className="kv"><span className="kv-l">  Lance Embutido</span><span className="kv-r">{R(sim.lvEmbutido)} ({Pt(p.grupo.lanceEmbutido)})</span></div>}
          <div className="kv"><span className="kv-l">Parcela Após Lance</span><span className="kv-r" style={{ color: 'var(--green)' }}>{R(sim.pl)}</span></div>
        </>}
        <div className="kv"><span className="kv-l">Custo Total</span><span className="kv-r">{R(sim.tl)}</span></div>
        {p.opcoes.cet && !isSimple && <>
          ...
          <div className="kv"><span className="kv-l">CET Mensal</span><span className="kv-r" style={{ color: "var(--gold)" }}>{Pt(sim.cetMensal, 3)} a.m.</span></div>
          <div className="kv"><span className="kv-l">CET Anual</span><span className="kv-r" style={{ color: "var(--gold)", fontSize: 15 }}>{Pt(sim.cetAnual, 2)} a.a.</span></div>
        </>}
      </div>

      {p.opcoes.comparativo && !isSimple && (
        <div className="sec-block">
          <div className="sec-block-title">Comparativo com Financiamento</div>
          <div className="kv"><span className="kv-l">Financiamento CEF (12,5% a.a.)</span><span className="kv-r">{R(sim.pmtCEF)}/mês</span></div>
          <div className="kv"><span className="kv-l">Total CEF</span><span className="kv-r">{R(sim.ttCEF)}</span></div>
          <div className="kv"><span className="kv-l" style={{ color: 'var(--green)' }}>Economia vs CEF</span><span className="kv-r" style={{ color: 'var(--green)' }}>{R(sim.ecCEF)}</span></div>
          {isAnalytic && <>
            <div className="kv"><span className="kv-l">Financiamento Banco (17,5% a.a.)</span><span className="kv-r">{R(sim.pmtBco)}/mês</span></div>
            <div className="kv"><span className="kv-l" style={{ color: 'var(--green)' }}>Economia vs Banco</span><span className="kv-r" style={{ color: 'var(--green)' }}>{R(sim.ecBco)}</span></div>
          </>}
        </div>
      )}

      {p.opcoes.incc && !isSimple && (
        <div className="sec-block">
          <div className="sec-block-title">Valorização do Bem (INCC 5,8% a.a.)</div>
          <div className="kv"><span className="kv-l">Valor Estimado em {sim.anos.toFixed(0)} anos</span><span className="kv-r" style={{ color: 'var(--green)' }}>{R(sim.cf)}</span></div>
          <div className="kv"><span className="kv-l">Valorização</span><span className="kv-r" style={{ color: 'var(--green)' }}>+{Pt(sim.inccPct)} ({R(sim.inccGanho)})</span></div>
        </div>
      )}
