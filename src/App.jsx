...
import {
  ...
  ChevronDown, MessageCircle, BarChart3, Calculator
} from "lucide-react";
...
export default function App() {
  ...
  const addNote = (negId, text) => {
    setActivities(p => [...p, { id: uid(), negotiationId: negId, channel: "Anotação", description: text, date: today() }]);
  };
          ...
          <div className="content" onClick={() => createOpen && setCreateOpen(false)}>
            ...
            {page === "negotiations" && selected && (
              <NegotiationDetail
                ...
                onWin={markWon}
                onLose={markLost}
                onAddActivity={(negId) => { setModalNegId(negId); setModal("newActivity"); }}
                onAddTask={(negId) => { setModalNegId(negId); setModal("newTaskFor"); }}
            ...
            {page === "contacts" && !selected && (
              <div className="fade-in">
                ...
                {contacts.length === 0
                  ? <div className="empty"><h3>Nenhum contato</h3><p>Crie o primeiro contato pelo botão acima.</p></div>
                  : <ContactList
