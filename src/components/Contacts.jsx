...
// All negotiations, tasks, history, annotations in one screen
// ═══════════════════════════════════════════════════════════════
export function ContactDetail({
  ...
  // Active negotiation (selected tab in negotiations panel)
  const activeNeg = negotiations.find(n => n.id === activeNegId) || negotiations[0] || null;
  const negActivities = activeNeg ? activities.filter(a => a.negotiationId === activeNeg.id).sort((a, b) => (b.date + b.id).localeCompare(a.date + a.id)) : [];
  const negTasks = activeNeg ? tasks.filter(t => t.negotiationId === activeNeg.id).sort((a, b) => a.date.localeCompare(b.date)) : [];
  const pendingTasks = negTasks.filter(t => !t.done);
  // All tasks across all negotiations for this contact
  const allContactTasks = tasks.filter(t => negotiations.some(n => n.id === t.negotiationId));
  const allPending = allContactTasks.filter(t => !t.done);
  // All activities across all negotiations
  const allContactActs = activities.filter(a => negotiations.some(n => n.id === a.negotiationId)).sort((a, b) => (b.date + b.id).localeCompare(a.date + a.id));

  const filteredActs = filterEvents === "all" ? (activeNeg ? negActivities : allContactActs)
    : filterEvents === "notes" ? (activeNeg ? negActivities : allContactActs).filter(a => a.channel === "Anotação")
    : filterEvents === "system" ? (activeNeg ? negActivities : allContactActs).filter(a => a.channel === "Sistema")
    : (activeNeg ? negActivities : allContactActs).filter(a => a.channel !== "Sistema" && a.channel !== "Anotação");
  ...
  const submitNote = () => {
    if (!noteText.trim() || !activeNeg) return;
    onAddNote(activeNeg.id, noteText, activeNeg.stage);
    setNoteText("");
  };
  const submitTask = () => {
    if (!taskTitle.trim() || !taskDate || !activeNeg) return;
    onAddTask(activeNeg.id, { title: taskTitle, description: taskDesc, date: taskDate });
    setTaskTitle(""); setTaskDate(""); setTaskDesc(""); setShowTaskForm(false);
  };
  const submitActivity = () => {
    if (!actDesc.trim() || !activeNeg) return;
    onAddActivity(activeNeg.id, { channel: actChannel, description: actDesc });
  ...
  const negScore = activeNeg ? calcScore(activeNeg, activities, tasks) : { score: 0, grade: "E" };
    ...
    <div className="fade-in">
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10, paddingBottom: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <span style={{ fontSize: 10.5, color: "var(--tx3)" }}>Contato · {contact.city || ""}{age !== null ? ` · ${age} anos` : ""}</span>
        </div>
      ...
      <div className="detail-layout">
        ...
        <div>
          ...
          <Panel title={`Negociações (${negotiations.length})`}>
            {negotiations.length === 0 ? (
              <p style={{ fontSize: 11.5, color: "var(--tx3)" }}>Nenhuma negociação.</p>
            ) : (
              negotiations.map(n => {
          ...
          {activeNeg && activeNeg.product === "Plano de Saúde" && activeNeg.health?.modality && (
            <Panel title={"Saúde — " + activeNeg.health.modality} defaultOpen={false}>
              ...
              {activeNeg.health.accommodation && <div className="panel-row"><span className="label">Acomodação</span><span className="value">{activeNeg.health.accommodation}</span></div>}
            </Panel>
          )}
          {activeNeg && activeNeg.product === "Consórcio" && activeNeg.consorcio?.branch && (
            <Panel title={"Consórcio — " + activeNeg.consorcio.branch} defaultOpen={false}>
              ...
              {activeNeg.consorcio.administrator && <div className="panel-row"><span className="label">Administradora</span><span className="value">{activeNeg.consorcio.administrator}</span></div>}
            </Panel>
          )}
        ...
        {/* ═══ RIGHT PANEL ═══ */}
        <div>
          {/* Pipeline bar for active negotiation */}
          {activeNeg && (<>
            ...
            <div className="pipeline-bar" style={{ marginBottom: 12 }}>
              {STAGES.map((stg, i) => {
                ...
                return (<div key={stg} className={`pipeline-step ${active ? "active" : ""} ${passed ? "passed" : ""}`}
                  ...
                  {stg}{active && <span className="days">({negDays} dias)</span>}
                </div>);
              })}
            </div>
          </>)}
          ...
          <div className="tasks-box">
            ...
            {pendingTasks.length === 0 ? <p style={{ fontSize: 12, color: "var(--tx3)", padding: "8px 0" }}>Nenhuma tarefa pendente{activeNeg ? " nesta negociação" : ""}.</p>
            : pendingTasks.map(t => {
              ...
              return (<div key={t.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 11px", marginBottom: 5, background: over ? "rgba(239,68,68,.05)" : "var(--s2)", border: "1px solid " + (over ? "rgba(239,68,68,.15)" : "var(--bd)"), borderRadius: 8 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 5 }}><span style={{ fontSize: 12, fontWeight: 500 }}>Tarefa</span>{over && <span style={{ background: "#EF4444", color: "#fff", fontSize: 9, fontWeight: 700, padding: "1px 5px", borderRadius: 3 }}>ATRASADA</span>}</div>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6 }}>
              <span style={{ fontSize: 10, color: "var(--tx3)" }}>Mostrando {pendingTasks.length}/{negTasks.length} tarefas</span>
              <button className="btn btn-sm" style={{ color: "var(--blu)", borderColor: "var(--blu)" }} onClick={() => setShowTaskForm(!showTaskForm)}><Plus size={10} /> Criar tarefa</button>
            </div>
            {showTaskForm && activeNeg && (<div style={{ marginTop: 8, padding: 10, background: "var(--s2)", border: "1px solid var(--bd)", borderRadius: 7 }}>
              <input value={taskTitle} onChange={e => setTaskTitle(e.target.value)} placeholder="Título da tarefa" style={{ marginBottom: 5 }} />
              <div className="row2" style={{ marginBottom: 5 }}><input type="datetime-local" value={taskDate} onChange={e => setTaskDate(e.target.value)} /><input value={taskDesc} onChange={e => setTaskDesc(e.target.value)} placeholder="Descrição (opcional)" /></div>
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 4 }}><button className="btn btn-sm" onClick={() => setShowTaskForm(false)}>Cancelar</button><button className="btn btn-primary btn-sm" onClick={submitTask}><Check size={10} /> Criar</button></div>
            </div>)}
          </div>
          ...
          <div className="timeline-area">
            ...
            {tab === "hist" && <>
              ...
              {/* Note form */}
              {activeNeg && (<div style={{ marginBottom: 12 }}>
                ...
                <div style={{ background: "var(--s2)", border: "1px solid var(--bd)", borderRadius: 7, padding: 10 }}>
                  <textarea ref={noteRef} value={noteText} onChange={e => setNoteText(e.target.value)} onKeyDown={handleNoteKey}
                    placeholder="Escreva uma anotação… (Ctrl+Enter para salvar)" style={{ width: "100%", minHeight: 60, background: "var(--s1)", border: "1px solid var(--bd)", borderRadius: 6, padding: "8px 10px", fontSize: 12.5, color: "var(--tx)", fontFamily: "inherit", resize: "vertical", outline: "none" }} />
              ...
              {/* Activity form */}
              {showActForm && activeNeg && (<div style={{ marginBottom: 12, padding: 10, background: "var(--s2)", border: "1px solid var(--bd)", borderRadius: 7 }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", gap: 4 }}><button className="btn btn-sm" onClick={() => setShowActForm(false)}>Cancelar</button><button className="btn btn-primary btn-sm" onClick={submitActivity}><Send size={10} /> Registrar</button></div>
              </div>)}
                  ...
                  <div key={a.id} className="timeline-item">
                    ...
                    <div className="timeline-content">
                      <div className="timeline-title">
                        ...
                        {isNote && <span style={{ color: "var(--tx2)", fontWeight: 400 }}> fez uma anotação na etapa {a.stage || neg?.stage || ""} do funil {neg?.name || ""}</span>}
            ...
            {tab === "tasks" && <>
              ...
              {negTasks.filter(t => !t.done).length === 0 ? <p style={{ fontSize: 12, color: "var(--tx3)", textAlign: "center", padding: 20 }}>Nenhuma tarefa pendente.</p>
              : negTasks.filter(t => !t.done).map(t => {
                const over = t.date && t.date.slice(0, 10) < today();
                return (<div key={t.id} style={{ padding: 11, marginBottom: 7, background: "var(--s2)", border: "1px solid var(--bd)", borderRadius: 7 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "flex-end", gap: 4, marginTop: 5 }}><button className="btn btn-sm btn-ghost" onClick={() => onToggleTask(t.id)}><Check size={10} color="var(--grn)" /> Concluir</button></div>
                </div>);
              })}
            </>}

            {/* ═══ PRODUTOS ═══ */}
            {tab === "product" && activeNeg && (<div style={{ fontSize: 12.5, color: "var(--tx2)", lineHeight: 1.8, padding: "8px 0" }}>
              ...
              {activeNeg.health?.branch && <><strong>Ramo:</strong> {activeNeg.health.branch}<br /></>}
              ...
              {activeNeg.consorcio?.buyerProfile && <><strong>Perfil:</strong> {activeNeg.consorcio.buyerProfile}<br /></>}
              <div style={{ marginTop: 10 }}><button className="btn btn-sm" style={{ color: "var(--blu)", borderColor: "var(--blu)" }}><Plus size={10} /> Adicionar produto ou serviço</button></div>
            </div>)}
