              ...
              <div className="fade-in" id="report-area">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                  ...
                  <button className="btn btn-sm" onClick={() => {
                    ...
                    setTimeout(() => { if (area) area.style.opacity = "1"; }, 100);
                  }}>Atualizar</button>
                  <div style={{ marginLeft: "auto", display: "flex", gap: 4 }}>
                    ...
                    <button className="btn btn-sm" onClick={() => {
                      ...
                      const fActs = activities.filter(a => (!from || a.date >= from) && (!to || a.date <= to));
                      const fNegs = negotiations.filter(n => (!from || n.createdAt >= from) && (!to || n.createdAt <= to));
                      let csv = "Data,Tipo,Contato,Negociação,Descrição\n";
                      fActs.forEach(a => {
                        const ct = contacts.find(c => c.id === a.contactId);
                        const ng = negotiations.find(n => n.id === a.negotiationId);
                        csv += `"${a.date}","${a.channel}","${ct?.name || ""}","${ng?.name || ""}","${(a.description || "").replace(/"/g, '""')}"\n`;
                      });
                      const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" });
                      const url = URL.createObjectURL(blob);
                      const a = document.createElement("a");
                      a.href = url; a.download = `relatorio-atividades-${from}-a-${to}.csv`; a.click();
                      URL.revokeObjectURL(url);
                ...
                {(() => {
                  ...
                  const fActs = activities.filter(a => (!from || a.date >= from) && (!to || a.date <= to));
                  const fNegs = negotiations.filter(n => (!from || n.createdAt >= from) && (!to || n.createdAt <= to));
                  const fWon = negotiations.filter(n => n.status === "won");
                  const fLost = negotiations.filter(n => n.status === "lost");
                  const fTasks = tasks;

                  // Group activities by day
                  const actByDay = {};
                  fActs.forEach(a => { const d = a.date || "sem-data"; actByDay[d] = (actByDay[d] || 0) + 1; });
                  const actDays = Object.entries(actByDay).sort((a, b) => b[0].localeCompare(a[0]));
                  ...
                  const tasksDone = fTasks.filter(t => t.done).length;
                  const tasksPending = fTasks.filter(t => !t.done).length;
                  ...
                  return (<>
                    ...
                    <div className="kpi-grid">
                      ...
                      <div className="kpi"><div className="kpi-label">Anotações</div><div className="kpi-value">{totalAnotacoes}</div><div className="kpi-sub">no período</div></div>
                      <div className="kpi"><div className="kpi-label">Total Atividades</div><div className="kpi-value">{fActs.length}</div><div className="kpi-sub">{actDays.length} dias ativos</div></div>
                      ...
                      <div className="kpi"><div className="kpi-label">Negociações Criadas</div><div className="kpi-value">{fNegs.length}</div><div className="kpi-sub">no período</div></div>
                      <div className="kpi"><div className="kpi-label">Tarefas Concluídas</div><div className="kpi-value" style={{ color: "var(--grn)" }}>{tasksDone}</div></div>
                      <div className="kpi"><div className="kpi-label">Tarefas Pendentes</div><div className="kpi-value" style={{ color: tasksPending > 0 ? "var(--red)" : "var(--grn)" }}>{tasksPending}</div></div>
                    </div>
                    ...
                    <div className="panel">
                      ...
                      <div className="panel-body" style={{ maxHeight: 400, overflowY: "auto" }}>
                        {fActs.length === 0 ? <p style={{ fontSize: 11, color: "var(--tx3)", padding: 10 }}>Nenhuma atividade no período.</p>
                        : fActs.sort((a, b) => (b.date + (b.id || "")).localeCompare(a.date + (a.id || ""))).map(a => {
                          ...
                          return (
                            <div key={a.id} style={{ display: "flex", gap: 8, padding: "7px 0", borderBottom: "1px solid var(--bd)", fontSize: 11.5 }}>
                              ...
                              {ng && <span style={{ color: "var(--tx3)" }}>{ng.name || ng.product}</span>}
