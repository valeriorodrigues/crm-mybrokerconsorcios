...
import {
  ...
  AGE_BANDS, CONSORCIO_BRANCHES, BUYER_PROFILES, BID_TYPES, RELATIONSHIPS
} from "../utils/constants";
import { uid, emptyNegotiation, emptyBeneficiary, calcAge } from "../utils/helpers";
import { Check, Plus, X, Star } from "lucide-react";

export function NegotiationForm({ negotiation, contacts, companies, savedOptions, onSave, onClose }) {
  const [f, setF] = useState(negotiation ? JSON.parse(JSON.stringify(negotiation)) : emptyNegotiation());
  ...
  const profiles = BUYER_PROFILES[co.branch] || BUYER_PROFILES["default"];

  // Beneficiaries
  const addBen = () => sH("beneficiaries", [...(h.beneficiaries || []), emptyBeneficiary()]);
  const updBen = (i, k, v) => {
    const b = [...h.beneficiaries]; b[i] = { ...b[i], [k]: v };
    if (k === "birthDate" && v) b[i].age = calcAge(v)?.toString() || "";
    sH("beneficiaries", b);
  };
  const rmBen = (i) => sH("beneficiaries", h.beneficiaries.filter((_, j) => j !== i));

  const stepLabels = ["Negociação", "Produto", "Detalhes"];

  return (
    <Modal title={negotiation ? "Editar Negociação" : "Nova Negociação"} onClose={onClose} wide
      footer={<>
      ...
      {step === 0 && <>
        ...
        <div className="row2">
          <Field label="Fonte de Origem">
            <AutoInput value={f.sourceOrigin} onChange={v => s("sourceOrigin", v)} suggestions={savedOptions.origins || []} placeholder="Ex: MyBroker Vinhedos" />
          </Field>
          <Field label="Indicação">
            <AutoInput value={f.sourceReferral} onChange={v => s("sourceReferral", v)} suggestions={savedOptions.referrals || []} />
          </Field>
        </div>
          ...
          <div className="row2">
            <Field label="Modalidade">
              <select value={h.modality} onChange={e => sH("modality", e.target.value)}>
                ...
                {HEALTH_MODALITIES.map(m => <option key={m}>{m}</option>)}
              </select>
            </Field>
            <Field label="Ramo do Plano">
              <select value={h.branch} onChange={e => sH("branch", e.target.value)}>
                <option value="">Selecionar…</option>
                {HEALTH_BRANCHES.map(b => <option key={b}>{b}</option>)}
              </select>
            </Field>
          </div>
                  ...
                  <tbody>
                    {h.beneficiaries.map((b, i) => (
                      <tr key={b.id}>
                        ...
                        {h.ageMethod === "band" && <td><select value={b.ageBand || ""} onChange={e => updBen(i, "ageBand", e.target.value)}><option value="">—</option>{AGE_BANDS.map(a => <option key={a}>{a}</option>)}</select></td>}
        ...
        {isC && <>
          ...
          <Field label="Ramo do Consórcio">
            <select value={co.branch} onChange={e => sC("branch", e.target.value)}>
              ...
              {CONSORCIO_BRANCHES.map(b => <option key={b}>{b}</option>)}
            </select>
          </Field>
      ...
      {step === 2 && <>
        {isH && h.modality && <>
          ...
          <div className="row2">
            ...
            {isAdesao
              ...
              : <Field label="Acomodação"><select value={h.accommodation || ""} onChange={e => sH("accommodation", e.target.value)}><option value="">—</option>{HEALTH_ACCOMMODATIONS.map(a => <option key={a}>{a}</option>)}</select></Field>
            }
          </div>
          ...
          {isAdesao && <Field label="Acomodação"><select value={h.accommodation || ""} onChange={e => sH("accommodation", e.target.value)}><option value="">—</option>{HEALTH_ACCOMMODATIONS.map(a => <option key={a}>{a}</option>)}</select></Field>}
          <Field label="Abrangência"><select value={h.coverageScope || ""} onChange={e => sH("coverageScope", e.target.value)}><option value="">—</option>{HEALTH_COVERAGES.map(c => <option key={c}>{c}</option>)}</select></Field>
        </>}
        ...
        {isC && co.branch && <>
          ...
          <div className="row2">
            <Field label="Tipo de Lance"><select value={co.bidType || ""} onChange={e => sC("bidType", e.target.value)}><option value="">—</option>{BID_TYPES.map(b => <option key={b}>{b}</option>)}</select></Field>
