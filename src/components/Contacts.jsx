...
import { RELATIONSHIPS, COMPANY_ROLES, STAGES, STAGE_COLORS, PRODUCT_COLORS, GRADE_COLORS, CHANNELS, FOLLOW_STATUSES, LEAD_ORIGIN_GROUPS, TAG_PALETTE } from "../utils/constants";
...
import {
  ...
  AlertTriangle, Pencil, User, XCircle, Trophy, Copy, Send, Target, Tag
} from "lucide-react";
...
const _TAG_PALETTE = TAG_PALETTE || ["#3B82F6","#10B981","#F59E0B","#EC4899","#8B5CF6"];
...
function TagInput({ tags = [], onChange }) {
  ...
  const color = (t) => _TAG_PALETTE[Math.abs(t.split("").reduce((a, c) => a + c.charCodeAt(0), 0)) % TAG_PALETTE.length];
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, padding: "6px 8px", background: "var(--s2)", border: "1px solid var(--brd)", borderRadius: 6, minHeight: 34, alignItems: "center" }}>
      {tags.map(t => (
        <span key={t} style={{ display: "inline-flex", alignItems: "center", gap: 3, padding: "2px 7px", borderRadius: 12, background: color(t) + "22", color: color(t), fontSize: 10, fontWeight: 600 }}>
          {t}<button onClick={() => remove(t)} style={{ background: "none", border: "none", cursor: "pointer", color: "inherit", padding: 0, lineHeight: 1 }}>×</button>
        </span>
      ))}
      <input value={input} onChange={e => setInput(e.target.value)}
        onKeyDown={e => { if (e.key === "Enter" || e.key === ",") { e.preventDefault(); add(input); } }}
        placeholder={tags.length === 0 ? "Digite e pressione Enter…" : ""}
        style={{ background: "none", border: "none", outline: "none", fontSize: 11, color: "var(--tx1)", minWidth: 120, flex: 1 }} />
    </div>
  );
}

// ─── Lead Origin Selector ───────────────────────────────────────────────────
function LeadOriginSelect({ value, custom, onChange, onCustomChange }) {
  const isCustom = value === "__custom__";
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      <select value={value || ""} onChange={e => onChange(e.target.value)} style={{ padding: "5px 8px", fontSize: 12, background: "var(--s2)", border: "1px solid var(--brd)", borderRadius: 6, color: "var(--tx1)" }}>
        <option value="">— Selecionar origem —</option>
        {_LEAD_ORIGIN_GROUPS.map(g => (
      ...
      {isCustom && (
        <input value={custom || ""} onChange={e => onCustomChange(e.target.value)}
          placeholder="Nome da fonte de origem" style={{ padding: "5px 8px", fontSize: 12, background: "var(--s2)", border: "1px solid var(--brd)", borderRadius: 6, color: "var(--tx1)" }} />
      )}
    </div>
  );
}
...
export function ContactForm({ contact, companies, allContacts, onSave, onClose }) {
  ...
  const followStatus = _FOLLOW_STATUSES.find(x => x.id === f.followStatus) || _FOLLOW_STATUSES[0];
  ...
  return (
    <Modal title={contact ? "Editar Contato" : "Novo Contato"} onClose={onClose}
      ...
      <div className="section-title" style={{ marginTop: 0 }}>Status de Acompanhamento</div>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 12 }}>
        {_FOLLOW_STATUSES.map(st => (
          <button key={st.id} onClick={() => s("followStatus", st.id)}
            style={{ padding: "4px 10px", borderRadius: 20, fontSize: 10.5, fontWeight: 600, cursor: "pointer",
              background: f.followStatus === st.id ? st.color + "33" : "var(--s2)",
              border: `1.5px solid ${f.followStatus === st.id ? st.color : "var(--brd)"}`,
              color: f.followStatus === st.id ? st.color : "var(--tx3)" }}>
            {st.icon} {st.label}
          </button>
        ))}
      </div>
      ...
      <LeadOriginSelect
        value={f.leadOrigin || ""}
        custom={f.leadOriginCustom || ""}
        onChange={v => s("leadOrigin", v)}
        onCustomChange={v => s("leadOriginCustom", v)}
      />

      {/* Tags */}
      <div className="section-title" style={{ marginTop: 12 }}>Tags</div>
      <TagInput tags={f.tags || []} onChange={v => s("tags", v)} />
...
export function ContactList({ contacts, companies, negotiations, onSelect, onDelete }) {
  ...
  const tagColor = (t) => _TAG_PALETTE[Math.abs(t.split("").reduce((a, c) => a + c.charCodeAt(0), 0)) % TAG_PALETTE.length];
  ...
  const filtered = contacts.filter(c => {
    ...
    const matchStatus = !filterStatus || c.followStatus === filterStatus;
    const matchOrigin = !filterOrigin
      || (c.leadOrigin || "").includes(filterOrigin)
      || (c.leadOriginCustom || "").includes(filterOrigin);
  ...
  return (<>
    <div style={{ display: "flex", gap: 6, marginBottom: 8, alignItems: "center", flexWrap: "wrap" }}>
      <div className="search-box">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar nome, telefone, email, cidade, tags…" style={{ width: 260 }} />
      </div>
      <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: "5px 8px", fontSize: 11 }}>
        <option value="">Todos os status</option>
        {_FOLLOW_STATUSES.map(s => <option key={s.id} value={s.id}>{s.icon} {s.label}</option>)}
      </select>
      <select value={filterOrigin} onChange={e => setFilterOrigin(e.target.value)} style={{ padding: "5px 8px", fontSize: 11 }}>
    ...
    <div className="table-wrap"><table><thead><tr><th>Contato</th><th>Status</th><th>Origem</th><th>Telefone</th><th>Tags</th><th>Negociações</th><th></th></tr></thead><tbody>
      ...
      : filtered.map(c => {
        ...
        const fs = _FOLLOW_STATUSES.find(x => x.id === c.followStatus);
        const origin = c.leadOrigin === "__custom__" ? c.leadOriginCustom : c.leadOrigin;
        return (<tr key={c.id} onClick={() => onSelect(c)}>
          <td><div style={{ display: "flex", alignItems: "center", gap: 8 }}><div className="contact-avatar">{(c.name || "?")[0].toUpperCase()}</div><div><span style={{ fontWeight: 500 }}>{c.name}</span>{c.city && <div style={{ fontSize: 10, color: "var(--tx3)" }}>{c.city}</div>}</div></div></td>
          <td>{fs ? <span style={{ padding: "2px 7px", borderRadius: 12, fontSize: 10, fontWeight: 600, background: fs.color + "22", color: fs.color, whiteSpace: "nowrap" }}>{fs.icon} {fs.label}</span> : "—"}</td>
          <td style={{ color: "var(--tx2)", fontSize: 11 }}>{origin || "—"}</td>
          <td style={{ color: "var(--tx2)" }}>{c.phone || "—"}</td>
          <td><div style={{ display: "flex", gap: 3, flexWrap: "wrap" }}>{(c.tags || []).slice(0, 3).map(t => <span key={t} style={{ padding: "1px 6px", borderRadius: 10, fontSize: 9, fontWeight: 600, background: tagColor(t) + "22", color: tagColor(t) }}>{t}</span>)}{(c.tags || []).length > 3 && <span style={{ fontSize: 9, color: "var(--tx3)" }}>+{c.tags.length - 3}</span>}</div></td>
          <td>{negs.length > 0 ? <div style={{ display: "flex", gap: 3 }}>{negs.slice(0,3).map(n => <span key={n.id} style={{ padding: "1px 5px", borderRadius: 8, fontSize: 9, background: (PRODUCT_COLORS[n.product] || "#888") + "22", color: PRODUCT_COLORS[n.product] || "#888" }}>{n.product?.split(" ")[0]}</span>)}</div> : <span style={{ color: "var(--tx3)", fontSize: 11 }}>—</span>}</td>
...
export function ContactDetail({
  ...
  onCreateNeg, onCreateNegInline, onEditNeg, onDeleteNeg,
  ...
  // ── Inline negotiation form state ────────────────────────────────────────
  const [showNegForm, setShowNegForm] = useState(false);
  const [negProduct, setNegProduct] = useState("Plano de Saúde");
  const [negName, setNegName] = useState("");
  const [negStage, setNegStage] = useState("Novos Leads");
  const [negValue, setNegValue] = useState("");

  const submitNegInline = () => {
    if (!negProduct) return;
    if (!onCreateNegInline) return;
    const name = negName.trim() || `${contact.name} — ${negProduct}`;
    onCreateNegInline({
      product: negProduct,
      name,
      stage: negStage,
    ...
    setNegProduct("Plano de Saúde");
    setNegName("");
    setNegStage("Novos Leads");
    setNegValue("");
    ...
    <div className="fade-in">
      ...
      <div className="detail-layout">
        ...
        <div>
          ...
          {/* Negotiations panel with inline create form */}
          <Panel title={`Negociações (${negotiations.length})`}>
            {negotiations.length === 0 && !showNegForm && (
              <p style={{ fontSize: 11, color: "var(--tx3)", marginBottom: 8 }}>Nenhuma negociação vinculada.</p>
            )}
            ...
            {/* ── Inline form ── */}
            {showNegForm ? (
              <div style={{ background: "var(--s2)", border: "1px solid var(--ac)", borderRadius: 8, padding: "10px 12px", marginTop: 8 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: "var(--ac)", marginBottom: 8 }}>Nova Negociação</div>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <select value={negProduct} onChange={e => setNegProduct(e.target.value)}
                    style={{ padding: "5px 8px", fontSize: 12, background: "var(--s1)", border: "1px solid var(--bd)", borderRadius: 6, color: "var(--tx1)" }}>
                    {PRODUCTS.map(p => <option key={p}>{p}</option>)}
                  </select>
                  <input value={negName} onChange={e => setNegName(e.target.value)}
                    placeholder={`Ex: ${contact.name?.split(" ")[0]} — ${negProduct}`}
                    style={{ padding: "5px 8px", fontSize: 12, background: "var(--s1)", border: "1px solid var(--bd)", borderRadius: 6, color: "var(--tx1)" }} />
                  <div style={{ display: "flex", gap: 6 }}>
                    <select value={negStage} onChange={e => setNegStage(e.target.value)}
                      style={{ flex: 1, padding: "5px 8px", fontSize: 12, background: "var(--s1)", border: "1px solid var(--bd)", borderRadius: 6, color: "var(--tx1)" }}>
                      {["Novos Leads","Andamento","Proposta Enviada","Fechamento"].map(s => <option key={s}>{s}</option>)}
                    </select>
                    <input value={negValue} onChange={e => setNegValue(e.target.value)} placeholder="Valor (R$)" type="number"
                      style={{ width: 100, padding: "5px 8px", fontSize: 12, background: "var(--s1)", border: "1px solid var(--bd)", borderRadius: 6, color: "var(--tx1)" }} />
                  </div>
                  <div style={{ display: "flex", gap: 5, justifyContent: "flex-end", marginTop: 2 }}>
                    <button className="btn btn-sm" onClick={() => setShowNegForm(false)}>Cancelar</button>
                    <button className="btn btn-primary btn-sm" onClick={submitNegInline}><Check size={11} /> Criar</button>
                  </div>
                </div>
              </div>
            ) : (
              <button className="btn btn-sm" style={{ color: "var(--blu)", borderColor: "var(--blu)", marginTop: 6, width: "100%" }}
