...
export const db = {
  ...
  // User settings (metas, ciclo)
  getSettings: async () => {
    const data = await supaFetch('settings?order=created_at.desc&limit=1');
    if (data && data[0]) {
      try { return JSON.parse(data[0].value); } catch { return null; }
    }
    return null;
  },
  saveSettings: async (settings) => {
    await supaFetch('settings', {
      method: 'POST',
      headers: { ...getHeaders(), 'Prefer': 'return=representation,resolution=merge-duplicates' },
      body: JSON.stringify({ id: 'user_settings', value: JSON.stringify(settings) }),
    });
  },
...
export const auth = {
  ...
  isLoggedIn() {
    ...
    // Validate JWT expiry (decode payload without library)
    try {
