...
function calcSim(g) {
  ...
  // Parcela pós-lance: o lance abate o saldo devedor, reduzindo a parcela proporcionalmente
  const creditoLiquido = credito - lv; // PV = o que o cliente "leva" de fato (para cálculo do CET)
  const saldoDevedorTotal = ps * prazo; // total nominal do contrato original
  const novoSaldo = saldoDevedorTotal - lv; // saldo após abatimento do lance
  const pl = lv > 0 ? (novoSaldo > 0 ? novoSaldo / prazo : 0) : ps; // nova parcela real após lance

  const plReduzida = reducao && reducaoPct > 0 ? pl * (1 - reducaoPct / 100) : pl;
  const tl = lv + pl * prazo; // custo total nominal = lv + novoSaldo = saldoDevedorTotal (lance não soma, só redistribui)
  const anos = prazo / 12;
...
function ProposalView({ proposal: p, onCopy }) {
  const grupos = p.grupos || (p.grupo ? [p.grupo] : []); // backward compat
  ...
  return (
    <div>
      ...
      {/* PLANO ESTRUTURADO — tabela de múltiplos grupos */}
      {isMulti ? (
        <div className="sec-block">
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(201,168,76,.3)' }}>
                  {['Grupo','Bem','Prazo','Parcela','Pós-Lance','Lance','CET a.a.'].map(h => (
                ...
                {grupos.map((g, idx) => {
                  ...
                  return (
                    <tr key={g._id || idx} style={{ borderBottom: '1px solid rgba(255,255,255,.04)' }}>
                      ...
                      <td style={{ padding: '7px 6px', textAlign: 'right', color: s.lv > 0 ? 'var(--green)' : 'var(--muted)' }}>{s.lv > 0 ? R(s.pl) : '—'}</td>
                      <td style={{ padding: '7px 6px', textAlign: 'right' }}>{s.lv > 0 ? R(s.lv) : '—'}</td>
                      <td style={{ padding: '7px 6px', textAlign: 'right', color: 'var(--gold)' }}>{s.cetAnual > 0 ? Pt(s.cetAnual, 2) : '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          return (
            <>
              <div className="sec-block">
                ...
                <div className="kv"><span className="kv-l">Prazo</span><span className="kv-r">{g.prazo} meses</span></div>
