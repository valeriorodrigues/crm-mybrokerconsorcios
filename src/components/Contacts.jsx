...
function TagInput({ tags = [], onChange }) {
  ...
  const color = (t) => TAG_PALETTE[Math.abs(t.split("").reduce((a, c) => a + c.charCodeAt(0), 0)) % TAG_PALETTE.length];
  return (
...
export function ContactForm({ contact, companies, allContacts, onSave, onClose }) {
  ...
  const followStatus = FOLLOW_STATUSES.find(x => x.id === f.followStatus) || FOLLOW_STATUSES[0];
...
export function ContactList({ contacts, companies, negotiations, onSelect, onDelete }) {
  ...
  const tagColor = (t) => TAG_PALETTE[Math.abs(t.split("").reduce((a, c) => a + c.charCodeAt(0), 0)) % TAG_PALETTE.length];
  ...
  return (<>
    <div style={{ display: "flex", gap: 6, marginBottom: 8, alignItems: "center", flexWrap: "wrap" }}>
      ...
      <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: "5px 8px", fontSize: 11 }}>
        ...
        {FOLLOW_STATUSES.map(s => <option key={s.id} value={s.id}>{s.icon} {s.label}</option>)}
      </select>
    ...
    <div className="table-wrap"><table><thead><tr><th>Contato</th><th>Status</th><th>Origem</th><th>Telefone</th><th>Tags</th><th>Negociações</th><th></th></tr></thead><tbody>
      ...
      : filtered.map(c => {
        ...
        const fs = FOLLOW_STATUSES.find(x => x.id === c.followStatus);
