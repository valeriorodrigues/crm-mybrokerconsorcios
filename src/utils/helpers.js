...
export function calcScore(negotiation, activities, tasks) {
  ...
  const stageBonus = {
    "Novos Leads": 0, "Andamento": 10, "Proposta Enviada": 20, "Fechamento": 30
  };
...
export const emptyContact = () => ({
  ...
  notes: "",
  createdAt: today(),
  // Follow-up status (acompanhamento)
  followStatus: "nao_atendido",
  // Tags livres
  tags: [],
  // Origem do lead
  leadOrigin: "",
...
export const emptyNegotiation = () => ({
  ...
  contactId: "",
  companyId: "",
  name: "", // e.g. "Diego César – Plano de Saúde"
...
export function sendNotification(title, body, onClick) {
  ...
  const n = new Notification(title, {
    ...
    icon: "/favicon.ico",
    badge: "/favicon.ico",
    tag: title, // deduplicates same-title notifs
  });
  if (onClick) n.onclick = onClick;
}

// Check tasks due today or overdue and fire notifications (call once per minute)
export function checkTaskNotifications(tasks, contacts, negotiations) {
  const todayStr = today();
  const due = tasks.filter(t => !t.done && t.date && t.date <= todayStr);
  due.forEach(t => {
    const ct = contacts.find(c => c.id === t.contactId);
    const ng = negotiations.find(n => n.id === t.negotiationId);
    const who = ct?.name || ng?.name || "Contato";
    const overdue = t.date < todayStr;
    sendNotification(
      overdue ? `⚠️ Tarefa atrasada: ${t.title}` : `🔔 Tarefa de hoje: ${t.title}`,
      `${who}${t.description ? " — " + t.description : ""}`,
    );
  });
}
