...
import {
  ...
  ChevronDown, MessageCircle, BarChart3, Calculator, Upload
} from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      const [ct, co, ng, ac, tk, tp] = await Promise.all([
        loadData("crm7-contacts", []),
        loadData("crm7-companies", []),
        loadData("crm7-negotiations", []),
        loadData("crm7-activities", []),
        loadData("crm7-tasks", []),
        loadData("crm7-templates", DEFAULT_TEMPLATES),
      ]);
      setContacts(ct); setCompanies(co); setNegotiations(ng);
      setActivities(ac); setTasks(tk); setTemplates(tp);
      setReady(true);
    })();
  }, []);

  useEffect(() => { if (ready) saveData("crm7-contacts", contacts); }, [contacts, ready]);
  useEffect(() => { if (ready) saveData("crm7-companies", companies); }, [companies, ready]);
  useEffect(() => { if (ready) saveData("crm7-negotiations", negotiations); }, [negotiations, ready]);
  useEffect(() => { if (ready) saveData("crm7-activities", activities); }, [activities, ready]);
  useEffect(() => { if (ready) saveData("crm7-tasks", tasks); }, [tasks, ready]);
  ...
  // ─── CRUD ───
  const saveContact = (c) => { setContacts(p => { const i = p.findIndex(x => x.id === c.id); return i >= 0 ? p.map(x => x.id === c.id ? c : x) : [...p, c]; }); setModal(null); setEditTarget(null); };
  const deleteContact = (id) => { if (!confirm("Excluir contato?")) return; setContacts(p => p.filter(x => x.id !== id)); setSelected(null); };

  const saveCompany = (c) => { setCompanies(p => { const i = p.findIndex(x => x.id === c.id); return i >= 0 ? p.map(x => x.id === c.id ? c : x) : [...p, c]; }); setModal(null); setEditTarget(null); };
  const deleteCompany = (id) => { if (!confirm("Excluir empresa?")) return; setCompanies(p => p.filter(x => x.id !== id)); setSelected(null); };

  const saveNeg = (n) => { setNegotiations(p => { const i = p.findIndex(x => x.id === n.id); return i >= 0 ? p.map(x => x.id === n.id ? n : x) : [...p, n]; }); setModal(null); setEditTarget(null); if (selected?.id === n.id) setSelected(n); };
  const deleteNeg = (id) => { if (!confirm("Excluir negociação?")) return; setNegotiations(p => p.filter(x => x.id !== id)); setActivities(p => p.filter(x => x.negotiationId !== id)); setTasks(p => p.filter(x => x.negotiationId !== id)); setSelected(null); };

  const saveActivity = (a) => { setActivities(p => [...p, a]); setModal(null); };
  const saveTask = (t) => { setTasks(p => { const i = p.findIndex(x => x.id === t.id); return i >= 0 ? p.map(x => x.id === t.id ? t : x) : [...p, t]; }); setModal(null); };
  ...
  const changeStage = (id, stage) => {
    ...
    setActivities(p => [...p, { id: uid(), negotiationId: id, channel: "Sistema", description: `Movido para ${stage}`, date: today() }]);
    if (selected?.id === id) setSelected(p => ({ ...p, stage }));
  };

  const markWon = (id) => {
    setNegotiations(p => p.map(n => n.id === id ? { ...n, status: "won", stage: "Fechamento" } : n));
    setActivities(p => [...p, { id: uid(), negotiationId: id, channel: "Sistema", description: "✅ Venda realizada!", date: today() }]);
  ...
  const markLost = (id) => {
    ...
    setActivities(p => [...p, { id: uid(), negotiationId: id, channel: "Sistema", description: `❌ Perda: ${reason || "Sem motivo"}`, date: today() }]);
    if (selected?.id === id) setSelected(p => ({ ...p, status: "lost", lostReason: reason }));
  };

  const addNote = (negId, text, stage) => {
    setActivities(p => [...p, { id: uid(), negotiationId: negId, channel: "Anotação", description: text, date: today(), stage: stage || "" }]);
  };
            ...
            {page === "contacts" && selected && (
              <ContactDetail
                ...
                onAddActivity={(contactId, negId, data) => {
                  if (data) {
                    setActivities(p => [...p, { id: uid(), contactId, negotiationId: negId || "", channel: data.channel, description: data.description, date: today() }]);
                  }
                }}
                onAddTask={(contactId, negId, data) => {
                  if (data) {
                    setTasks(p => [...p, { id: uid(), contactId, negotiationId: negId || "", title: data.title, description: data.description || "", date: data.date, done: false, createdAt: today() }]);
                    setActivities(p => [...p, { id: uid(), contactId, negotiationId: negId || "", channel: "Sistema", description: `Tarefa criada: ${data.title}`, date: today() }]);
                  }
                }}
                onToggleTask={toggleTask}
                onAddNote={(contactId, negId, text, stage) => {
                  setActivities(p => [...p, { id: uid(), contactId, negotiationId: negId || "", channel: "Anotação", description: text, date: today(), stage: stage || "" }]);
                }}
              />
            )}
